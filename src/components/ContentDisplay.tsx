...
interface ContentDisplayProps {
  ...
  onGenerateELI5: (topic: string) => void;
...
const ActionButton = ({ icon, text, onClick, isLoading }: { icon: React.ReactNode, text: string, onClick: () => void, isLoading: boolean }) => (
    <button
        onClick={onClick}
        disabled={isLoading}
        className="relative flex flex-col items-center justify-center text-center p-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-blue-500 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-950 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
    >
...
export function ContentDisplay({ 
    error, selectedTopic, isCourseSelected, content, activeView, speechState, onGenerateAnswer, onGenerateImage, onGenerateMCQs, onGenerateELI5, onToggleSpeech
}: ContentDisplayProps): React.ReactNode {
  
  const actionsContainerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // When a new topic is selected, automatically "swipe" (scroll) to the action buttons.
    if (selectedTopic && actionsContainerRef.current) {
      // A small timeout helps ensure the element is fully rendered and visible before scrolling,
      // which is helpful when dealing with CSS animations.
      const timer = setTimeout(() => {
        actionsContainerRef.current?.scrollIntoView({
          behavior: 'smooth',
          block: 'center', // This will align the actions container to the center of the viewport.
        });
      }, 150); 
      
      return () => clearTimeout(timer); // Cleanup timer on re-render
    }
  }, [selectedTopic]); // This effect runs only when the `selectedTopic` prop changes.
  ...
  return (
    <div key={selectedTopic} className="space-y-8 animate-fade-in">
      ...
      <div ref={actionsContainerRef} className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <ActionButton
          ...
          />
           <ActionButton
            ...
            isLoading={content?.eli5.isLoading ?? false}
            onClick={() => onGenerateELI5(selectedTopic)}
          />
          <ActionButton
      ...
      <div className="space-y-6">
        ...
        {activeView === 'eli5' && content?.eli5.data && <AnswerCard title="Simplified Explanation (ELI5)" content={content.eli5.data} />}
        ...
        {activeView === 'eli5' && content?.eli5.error && <ErrorMessage message={content.eli5.error} />}
